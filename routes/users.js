...
router.get('/userlist',function (req,res,next) {
  User.find({},function(err,rtn){
    if(err) throw err;
    res.render('user/userlist',{title:'User List',users:rtn})
  })
})
...
router.post('/userupdate',function(req,res,next){
  ...
  User.findByIdAndUpdate(req.body.id,{$set:updatep},function(err,rtn){
    if(err) throw err;
    res.redirect('/users/userlist')
  })
})
  router.get('/userdetail/:id',function(req,res,next){
    User.findById(req.params.id,function (err,rtn) {
      if(err) throw err;
      res.render('user/user-details',{title:'User Detail',users:rtn})
  })
  })
